...
module RhVec = 
    ...
    let inline projectToXYPlane (v:Vector3d) = 
        ...
        if r.IsTiny(RhinoMath.SqrtEpsilon) then RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec.projectToXYPlane: Cannot projectToXYPlane for vertical vector %A" v
        r
    ...
    let projectToLine (ln:Line) (v:Vector3d) (pt:Point3d) = 
        ...
        if (a-b).SquareLength > RhinoMath.ZeroTolerance then
            ...
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec.projectToLine: missed Line by: %g " (a-b).Length
        a
    ...
    let slopePercent (v:Vector3d) = 
        if abs(v.Z) < RhinoMath.SqrtEpsilon then RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec.slopePercent: Can't get Slope from vertical vector %A" v
        let f = Vector3d(v.X, v.Y, 0.0)
        100.0 * (v.Z/f.Length)
    ...
    let inline setLength (len:float) (v:Vector3d) = 
        let l  = v.SquareLength
        if l < RhinoMath.SqrtEpsilon then RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec.setLength: Cant set length of tiny vector %A" v
    ...
    let inline perpendicularVecInXY (v:Vector3d) = 
        ...
        if len < RhinoMath.SqrtEpsilon then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec.perpendicularVecInXY: Cannot find perpendicularVecInXY for vertical vector %A" v
        else
            Vector3d(x, y, 0.0)
    ...
    let inline perpendicularVecInVerticalPlane (v:Vector3d) = 
        ...
        let r = cross v hor
        if r.IsTiny(RhinoMath.SqrtEpsilon) then RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec.perpendicularVecInVerticalPlane: Cannot find perpendicularVecInVerticalPlane for vertical vector %A" v
    ...
    let isAngleBelow1Degree(a:Vector3d, b:Vector3d) = //(prevPt:Point3d, thisPt:Point3d, nextPt:Point3d) = 
        ...
        if sa < RhinoMath.SqrtEpsilon then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelow1Degree: prevPt - thisPt: %s.SquareLength < RhinoMath.SqrtEpsilon; nextPt - thisPt:%s " a.ToNiceString b.ToNiceString
        let sb = b.SquareLength
        if sb < RhinoMath.SqrtEpsilon then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelow1Degree: nextPt - thisPt: %s.SquareLength < RhinoMath.SqrtEpsilon; prevPt - thisPt:%s " b.ToNiceString a.ToNiceString
        let lena = sqrt sa
        let lenb = sqrt sb
        if lena < Scripting.Doc.ModelAbsoluteTolerance then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelow1Degree: prevPt - thisPt: %s < Scripting.Doc.ModelAbsoluteTolerance: %f; nextPt - thisPt:%s " a.ToNiceString Scripting.Doc.ModelAbsoluteTolerance b.ToNiceString
        if lenb < Scripting.Doc.ModelAbsoluteTolerance then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelow1Degree: nextPt - thisPt: %s < Scripting.Doc.ModelAbsoluteTolerance: %f; prevPt - thisPt:%s " b.ToNiceString Scripting.Doc.ModelAbsoluteTolerance a.ToNiceString
        let au = a * (1.0 / lena)
        let bu = b * (1.0 / lenb)
    ...
    let isAngleBelowQuaterDegree(a:Vector3d, b:Vector3d) = //(prevPt:Point3d, thisPt:Point3d, nextPt:Point3d) = 
        ...
        let sa = a.SquareLength
        if sa < RhinoMath.SqrtEpsilon then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelowQuaterDegree: prevPt - thisPt: %s.SquareLength < RhinoMath.SqrtEpsilon; nextPt - thisPt:%s " a.ToNiceString b.ToNiceString
        let sb = b.SquareLength
        if sb < RhinoMath.SqrtEpsilon then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelowQuaterDegree: nextPt - thisPt: %s.SquareLength < RhinoMath.SqrtEpsilon; prevPt - thisPt:%s " b.ToNiceString a.ToNiceString
        let lena = sqrt sa
        let lenb = sqrt sb
        if lena < Scripting.Doc.ModelAbsoluteTolerance then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelowQuaterDegree: prevPt - thisPt: %s < Scripting.Doc.ModelAbsoluteTolerance: %f; nextPt - thisPt:%s " a.ToNiceString Scripting.Doc.ModelAbsoluteTolerance b.ToNiceString
        if lenb < Scripting.Doc.ModelAbsoluteTolerance then
            RhinoScriptingException.Raise "Rhino.Scripting.Extra.RhVec Duplicate points: isAngleBelowQuaterDegree: nextPt - thisPt: %s < Scripting.Doc.ModelAbsoluteTolerance: %f; prevPt - thisPt:%s " b.ToNiceString Scripting.Doc.ModelAbsoluteTolerance a.ToNiceString
        let au = a * (1.0 / lena)
        let bu = b * (1.0 / lenb)
