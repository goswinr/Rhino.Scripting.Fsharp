...
type internal rhsy = RhinoScriptSyntax
...
module AutoOpenCurried =
  ...
  type RhinoScriptSyntax with
    ...
    static member setLayer (layer:string) (objectId:Guid) : unit =
        rhsy.ObjectLayer(objectId, layer, createLayerIfMissing=true)
    ...
    static member setLayers (layer:string) (objectIds:seq<Guid>) : unit =
        rhsy.ObjectLayer(objectIds,layer, createLayerIfMissing=true)
    ...
    static member getLayerShort (objectId:Guid) : string = 
        rhsy.ObjectLayerShort(objectId)
    ...
    static member setNames (name:string) (objectIds:seq<Guid>) : unit = 
        rhsy.ObjectName(objectIds, name)  
    ...
    static member setColor(color:Drawing.Color) (objectId:Guid) : unit = 
        rhsy.ObjectColor(objectId, color)  
    ...
    static member setColors(color:Drawing.Color) (objectIds:seq<Guid>) : unit = 
        rhsy.ObjectColor(objectIds, color)  
    ...
    static member setUserTexts( key:string) ( value :string) (objectIds:seq<Guid>) : unit = 
        rhsy.SetUserText(objectIds, key, value)
    
    ///<summary>Append a string to a possibly already existing usertext value.</summary>
    ///<param name="key">(string) The key name to set</param>
    ///<param name="value">(string) The string value to append. Cannot be empty string. use rhsy.DeleteUserText to delete keys</param>
    ...
    static member appendtUserText(key:string) (value :string) (objectId:Guid) : unit = 
        if String.IsNullOrWhiteSpace key then RhinoScriptingException.Raise "RhinoScriptSyntax.appendtUserText key is String.IsNullOrWhiteSpace for value  %s on %s" value (toNiceString objectId)
        if isNull value then RhinoScriptingException.Raise "RhinoScriptSyntax.appendtUserText value is null  for key %s on %s" key (toNiceString objectId)        
        ...
        if isNull existing then // only if a value already exist  appending a white space  or empty string is OK too
            if String.IsNullOrWhiteSpace value  then RhinoScriptingException.Raise "RhinoScriptSyntax.appendtUserText failed on %s for key '%s' but value IsNullOrWhiteSpace" (toNiceString objectId) key
            if not <| obj.Attributes.SetUserString(key, value) then RhinoScriptingException.Raise "RhinoScriptSyntax.appendtUserText failed on %s for key '%s' and value '%s'" (toNiceString objectId) key value
        else 
            if not <| obj.Attributes.SetUserString(key,  existing + value ) then RhinoScriptingException.Raise "RhinoScriptSyntax.appendtUserText failed on %s for key '%s' and value '%s'" (toNiceString objectId) key value
    ...
    static member isUserTextValue( key:string) (valueToMatch:string) (objectId:Guid) : bool = 
        valueToMatch = rhsy.GetUserText(objectId, key)
    ...
    static member tryGetUserText( key:string) (objectId:Guid) : string option= 
        rhsy.TryGetUserText(objectId, key)
    ...
    static member matchAllUserText (sourceId:Guid) (targetId:Guid) : unit= 
        let sc = rhsy.CoerceRhinoObject(sourceId)
        let de = rhsy.CoerceRhinoObject(targetId)
        ...
        for  i = 0 to usg.Count-1 do 
            ...
            if not <|de.Geometry.SetUserString(key,sc.Geometry.GetUserString(key)) then 
                RhinoScriptingException.Raise "RhinoScriptSyntax.matchAllUserText: Geometry failed to set key '%s' from %s on %s" key (toNiceString sourceId) (toNiceString targetId)
        ...
        for  i = 0 to usa.Count-1 do 
            ...
            if not <|de.Attributes.SetUserString(key,sc.Attributes.GetUserString(key))then 
                RhinoScriptingException.Raise "RhinoScriptSyntax.matchAllUserText: Attributes failed to set key '%s' from %s on %s" key (toNiceString sourceId) (toNiceString targetId)
    ...
    static member matchUserText (sourceId:Guid) ( key:string) (targetId:Guid) : unit= 
        let de = rhsy.CoerceRhinoObject(targetId)
        let v = rhsy.GetUserText(sourceId,key)
        if not <| de.Attributes.SetUserString(key,v) then RhinoScriptingException.Raise "RhinoScriptSyntax.matchUserText: failed to set key '%s' to '%s' on %s" key v (toNiceString targetId) 
    ...
    static member matchName (sourceId:Guid) (targetId:Guid) : unit = 
        ...
        if isNull n then RhinoScriptingException.Raise "RhinoScriptSyntax.matchName: scource object %s has no name. Targets name: '%s'" (toNiceString sourceId) de.Attributes.Name
        de.Attributes.Name <- n
        if not <| de.CommitChanges() then RhinoScriptingException.Raise "RhinoScriptSyntax.matchName failed from %s on %s" (toNiceString sourceId) (toNiceString targetId)
    ...
    static member matchLayer (sourceId:Guid) (targetId:Guid) : unit = 
        ...
        if not <| de.CommitChanges() then RhinoScriptingException.Raise "RhinoScriptSyntax.matchLayer failed from %s on %s" (toNiceString sourceId) (toNiceString targetId)
    ...
    static member matchAllProperties (sourceId:Guid) (targetId:Guid) : unit = 
        ...
        if not <| de.CommitChanges() then RhinoScriptingException.Raise "RhinoScriptSyntax.matchAllProperties failed from %s on %s" (toNiceString sourceId) (toNiceString targetId)
        let usg = sc.Geometry.GetUserStrings()
        for  i = 0 to usg.Count-1 do 
            let key = usg.GetKey(i)
            if not <|de.Geometry.SetUserString(key,sc.Geometry.GetUserString(key)) then 
                RhinoScriptingException.Raise "RhinoScriptSyntax.matchAllProperties: Geometry failed to set key '%s' from %s on %s" key (toNiceString sourceId) (toNiceString targetId)
    ...
    static member draw (layer:string) (geo:'AnyRhinoGeometry) : unit =  
        rhsy.Add(geo) |> rhsy.setLayer layer
    ...
    static member transform (matrix:Transform) (objectId:Guid) : Guid =  
        rhsy.TransformObject(objectId, matrix, copy=false)       
    ...
    static member scale(origin:Point3d) (scale:float) (objectId:Guid) : unit = 
        ...
        let res = rhsy.Doc.Objects.Transform(objectId, xf, deleteOriginal=true)
        if res = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.scale failed. objectId:'%s' origin:'%s' scale:'%g'" (toNiceString objectId) origin.ToNiceString scale
    ...
    static member move (translation:Vector3d)  (objectId:Guid): unit = 
        ...
        let res = rhsy.Doc.Objects.Transform(objectId, xf, deleteOriginal=true) // TODO test to ensure GUID is the same ?
        if res = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.move to from objectId:'%s' translation:'%A'" (toNiceString objectId) translation
    ...
    static member moveX (translationX:float)  (objectId:Guid): unit = 
        ...
        let res = rhsy.Doc.Objects.Transform(objectId, xf, deleteOriginal=true) // TODO test to ensure GUID is the same ?
        if res = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.moveX to from objectId:'%s' translation:'%A'" (toNiceString objectId) translationX
    ...
    static member moveY (translationY:float)  (objectId:Guid): unit = 
        ...
        let res = rhsy.Doc.Objects.Transform(objectId, xf, deleteOriginal=true) // TODO test to ensure GUID is the same ?
        if res = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.moveY to from objectId:'%s' translation:'%A'" (toNiceString objectId) translationY
    ...
    static member moveZ (translationZ:float)  (objectId:Guid): unit = 
        ...
        let res = rhsy.Doc.Objects.Transform(objectId, xf, deleteOriginal=true) // TODO test to ensure GUID is the same ?
        if res = Guid.Empty then RhinoScriptingException.Raise "RhinoScriptSyntax.moveZ to from objectId:'%s' translation:'%A'" (toNiceString objectId) translationZ
