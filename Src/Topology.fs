...
module Topology =
        ...
        let sortToLoop(getLine: 'T -> Line) (xs:Rarr<'T>)  = 
            for i = 0 to xs.Count - 2 do // only run till second last
                ...
                let nextIdx = xs |> minIndexByFrom (fun c -> rhsy.DistanceSquare ((getLine c).From ,  thisLine.To) ) (i+1)
                xs |> swap (i+1) nextIdx
        ...
        let sortToLoopWithReversing (getLine: 'T -> Line) (reverseInPlace: int -> 'T -> unit) (xs:Rarr<'T>) : unit = 
            for i = 0 to xs.Count - 2 do // only run till second last
                let thisLine = getLine xs.[i] 
                // could be optimised using a R-Tree for very large lists instead of minBy function
                let nextIdxSt = xs |> minIndexByFrom (fun c -> rhsy.DistanceSquare ((getLine c).From ,  thisLine.To) ) (i+1)
                let nextIdxEn = xs |> minIndexByFrom (fun c -> rhsy.DistanceSquare ((getLine c).To   ,  thisLine.To) ) (i+1)
                // check if closest endpoint is closer than closest startpoint 
                if  rhsy.DistanceSquare ((getLine xs.[nextIdxSt]).From ,  thisLine.To) <=  
                    rhsy.DistanceSquare ((getLine xs.[nextIdxEn]).To   ,  thisLine.To) then 
