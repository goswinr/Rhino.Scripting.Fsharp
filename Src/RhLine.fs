...
module RhLine = 
    ...
    let offset amount (ln:Line) = 
        ...
        if lenXY  < 1e-9 then RhinoScriptingException.Raise "Rhino.Scripting.Extension: Line.offset: Cannot offset vertical Line  (by %g) %s" amount ln.ToNiceString
    ...
    let divide (segments:int) (ln:Line) = 
        match segments with
        | x when x < 1 -> RhinoScriptingException.Raise "Rhino.Scripting.Extension.Line.divide failed for %d segments. Minimum one. for %s"  segments ln.ToNiceString
        | 1 -> [|ln.From;  ln.To|]
        | k ->
    ...
    let intersectInOnePoint (lnA:Line) (lnB:Line) : Point3d = 
        ...
        if not ok then RhinoScriptingException.Raise "Rhino.Scripting.Extension.Line.intersectInOnePoint failed, parallel ?  on %s and %s" lnA.ToNiceString lnB.ToNiceString
        let a = lnA.PointAt(ta)
        let b = lnB.PointAt(tb)
        if (a-b).SquareLength > RhinoMath.ZeroTolerance then // = Length > 1e-6
            RhinoScriptingException.Raise "Rhino.Scripting.Extension.Line.intersect intersectInOnePoint, they are skew. distance: %g  on %s and %s" (a-b).Length lnA.ToNiceString lnB.ToNiceString
        b
    ...
    let intersectSkew (lnA:Line) (lnB:Line) :Point3d*Point3d= 
        let ok, ta, tb = Intersect.Intersection.LineLine(lnA,lnB)
        if not ok then RhinoScriptingException.Raise "Rhino.Scripting.Extension.Line.intersectSkew failed, parallel ?  on %s and %s" lnA.ToNiceString lnB.ToNiceString
        let a = lnA.PointAt(ta)
        let b = lnB.PointAt(tb)
        a,b
    ...
    let intersectFinite (lnA:Line) (lnB:Line) : Point3d[]= 
        let ok, ta, tb = Intersect.Intersection.LineLine(lnA,lnB)
        if not ok then [||] //RhinoScriptingException.Raise "Rhino.Scripting.Extension.Line.intersectFinite failed, parallel ?  on %s and %s" lnA.ToNiceString lnB.ToNiceString
        else
            let ca = clamp 0. 1. ta
            let cb = clamp 0. 1. tb
