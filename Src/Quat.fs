...
type [<Struct>] Quat = 
    ...
    private new (x, y, z, w) = // assume  unitized ??   // make private ??
        if abs(x*x+y*y+z*z+w*w-1.0) > 1e-9 then RhinoScriptingException.Raise "Rhino.Scripting.Extra.Quat constructors are not unitized: %g %g %g %g" x y z w // skip check ??
    ...
    static member divideBy k (q:Quat)  =  if abs k > 1e-8 then q.ScaleBy (1./k) else RhinoScriptingException.Raise "Rhino.Scripting.Extra.Quat: Cannot devide Quat %A by %f" q k
    ...
    //static member failIfNotNorm (q:Quat) = if q |> Quat.isNormalized |> not then RhinoScriptingException.Raise "Rhino.Scripting.Extra.%A is not Unitized" q else q

    static member fromXYZW (x, y, z, w) = 
        let l = sqrt ( x*x + y*y + z*z + w*w )
        if l < 1e-9 then RhinoScriptingException.Raise "Rhino.Scripting.Extra.Quat.fromXYZW failed on : x:%g y:%g z:%g w:%g" x y z w
        let lq = 1. / l
    ...
    //static member inline ( / ) (q:Quat,f:float) = if abs f < 1e-9 then RhinoScriptingException.Raise "Rhino.Scripting.Extra.Cannot devide Quat %A by %f" q f  else q * (1./f)
    ...
    static member fromVecRad (axis:Vector3d, angleRadian) = 
        ...
        if li < 1e-8 then RhinoScriptingException.Raise "Rhino.Scripting.Extra.Quat.fromVecRad: Cannot create Quat to rotate %g radians around zero length vector %A" angleRadian axis //or return identity ?
    ...
    static member fromVecToVec (u:Vector3d,v:Vector3d) = 
        ...
        if n.SquareLength < 1e-12 then // vectors are paralell
            if u.IsTiny 1e-8  then RhinoScriptingException.Raise "Rhino.Scripting.Extra.Quat.fromVecToVec u: %A (+ %A) to short in Quat.fromVecToVec" u v
            if v.IsTiny 1e-8  then RhinoScriptingException.Raise "Rhino.Scripting.Extra.Quat.fromVecToVec v: %A (+ %A) to short in Quat.fromVecToVec" v u
            if u*v > 0. then
